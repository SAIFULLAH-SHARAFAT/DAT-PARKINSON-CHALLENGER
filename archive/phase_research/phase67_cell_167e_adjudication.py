...
def adjudicate(
    ...
    # KNOWN DEFECT (see docs/EXPERIMENT_HISTORY.md, Phase67): seed_vectors
    # holds the raw per-seed expert probabilities, not the per-seed blends,
    # so this comparison is against the wrong quantity and cannot be
    # satisfied. The mismatch is reported rather than silently failing.
    seed_basis = "raw_per_seed_expert_not_per_seed_blend"
    ...
    criteria = {
        "seed_comparison_basis": seed_basis,
...
def main():
    ...
    # Fail closed: an unproven cache parity must block a cache-trained
    # candidate, not wave it through because Phase67A was never run.
    parity_blocking = True
    parity_status = None
    ...
    # phase67b is cache-trained too: 167c instructs the operator to copy the
    # multiseed expert into the artifact root and rerun the stack, so the
    # stack inherits the cache dependency it cannot otherwise declare.
    cache_trained = {"phase67b", "phase67c"}
    if best is None:
