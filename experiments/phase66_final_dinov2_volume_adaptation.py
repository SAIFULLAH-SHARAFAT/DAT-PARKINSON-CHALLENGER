...
CONFIG = {
    ...
    "output_root": os.environ.get("DAT_OUTPUT_ROOT", "/kaggle/working/phase66_final_private"),
...
ALGORITHM_VERSION = 'phase66_dinov2_volume_lora_v2_verified_wrapped_weights'
...
def fit_one(base, cache, labels, groups, folds, fold, seed_index, output, run_hash, config, device, tiny=False):
    ...
    for epoch in range(start_epoch, config['epochs']):
        if training_spent(output, run_hash) >= config['training_budget_hours'] * 3600:
            raise Phase65BRestoreStop('fixed_training_budget_exhausted_no_candidate_promotion')
