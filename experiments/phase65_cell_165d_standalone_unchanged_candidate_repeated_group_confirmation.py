    ...
    PHASE65C_CONFIG = {
        **PHASE65C_CONFIG,
        ...
        "contract_file": str(phase65c_test_override.get(
            "contract_file", "/tmp/phase65c_synthetic_contract.json"
        )),
        "checkpoint_directory": str(phase65c_test_override.get(
            "checkpoint_directory", "/tmp/phase65c_private_checkpoint"
        )),
    }
...
# ---------------------------------------------------------------------------
# Phase65D — unchanged Phase65C repeated whole-acquisition-group confirmation
# ---------------------------------------------------------------------------
#
# The complete Phase65C restoration boundary, model, losses, augmentation,
# optimizer, 14 epochs, EMA, adversarial strength, and 0.85/0.15 anchor blend
# above are copied unchanged into this standalone cell. Phase65D changes only
# the evaluation constitution: it uses the five label-blind group-reassignment
# maps frozen by Phase62 before Phase65C existed. No Phase65C outer prediction
# is used to fit, select, calibrate, route, or alter this candidate.

phase65d_started = time.perf_counter()

PHASE65D_CONFIG = {
    "schema_version": "phase65d_unchanged_phase65c_repeated_group_confirmation_v1",
    "repeat_count": 5,
    "fold_count": 3,
    "major_groups": [1, 3],
    "domain_bootstrap_replicates": 10000,
    "domain_bootstrap_seed": 650366,
    "advancement_gate": {
        "minimum_averaged_oof_log_loss_gain": 0.003,
        "minimum_averaged_oof_auroc_gain": 0.001,
        "minimum_repeat_log_loss_wins": 4,
        "minimum_repeat_auroc_wins": 3,
        "maximum_repeat_log_loss_regret": 0.002,
        "maximum_individual_fold_log_loss_regret": 0.005,
        "minimum_major_groups_combined_log_loss_gain": 0.003,
        "maximum_individual_major_group_log_loss_regret": 0.0,
        "minimum_domain_bootstrap_lower_95_log_loss_gain": 0.0,
        "brier_regret_allowed": 0.0,
    ...
    "checkpoint_directory": str(
        PHASE65B_WORKING_ROOT / "phase65d_private_checkpoint"
    ),
    "contract_file": str(
        PHASE65B_WORKING_ROOT
        / "phase65d_unchanged_candidate_repeated_group_confirmation_contract.json"
    ),
}
...
if phase65c_is_test:
    PHASE65D_CONFIG = {
        **PHASE65D_CONFIG,
        "repeat_count": int(phase65c_test_override.get("repeat_count", 2)),
        "domain_bootstrap_replicates": int(
            phase65c_test_override.get("domain_bootstrap_replicates", 200)
        ),
        "checkpoint_directory": str(phase65c_test_override.get(
            "phase65d_checkpoint_directory",
            "/tmp/phase65d_private_checkpoint",
        )),
        "contract_file": str(phase65c_test_override.get(
            "phase65d_contract_file",
            "/tmp/phase65d_synthetic_contract.json",
        )),
    }


# Recover and verify the exact Phase65C failure contract. A near-miss remains a
# failure; this cell performs a new robustness adjudication and does not rewrite
# the original decision.
phase65d_phase65c_contract_sha256 = None
if not phase65c_is_test:
    phase65d_phase65c_contract = phase65b_json(
        PHASE65B_WORKING_ROOT
        / "phase65c_domain_invariant_bilateral_3d_gate_contract.json",
        "phase65c_contract",
    )
    phase65b_require(
        phase65d_phase65c_contract.get("status")
        == "phase65c_domain_invariant_gate_failed_stop_candidate",
        "phase65c_failure_decision_not_preserved",
    )
    phase65b_require(
        phase65b_contract_core_hash_valid(phase65d_phase65c_contract),
        "phase65c_contract_hash_mismatch",
    )
    phase65b_require(
        phase65d_phase65c_contract.get("frozen_run_sha256")
        == phase65c_run_sha256,
        "phase65c_frozen_run_link_mismatch",
    )
    phase65d_phase65c_contract_sha256 = str(
        phase65d_phase65c_contract["contract_sha256"]
    )


# Reconstruct the five canonical Phase62 group maps directly from the accepted
# contract. These assignments were generated without labels and frozen before
# the Phase65C candidate was defined.
if phase65c_is_test:
    supplied_repeat_group_to_fold = phase65c_test_override.get(
        "repeat_group_to_fold"
    )
    if supplied_repeat_group_to_fold is None:
        base = np.arange(phase65c_expected_group_count, dtype=np.int64)
        supplied_repeat_group_to_fold = np.stack([
            (base + repeat) % 3
            for repeat in range(PHASE65D_CONFIG["repeat_count"])
        ])
    phase65d_group_to_fold = np.asarray(
        supplied_repeat_group_to_fold, dtype=np.int64
    )
else:
    phase65d_phase62_contract = phase65b_json(
        PHASE65B_WORKING_ROOT
        / "phase62_acquisition_domain_validation_contract.json",
        "phase62_repeat_contract",
    )
    phase65b_require(
        phase65b_contract_core_hash_valid(phase65d_phase62_contract),
        "phase62_repeat_contract_hash_mismatch",
    )
    phase65b_require(
        phase65d_phase62_contract.get("contract_sha256")
        == phase65c_validation["contract_sha256"],
        "phase62_repeat_contract_link_mismatch",
    )
    phase65d_group_to_fold = np.asarray(
        phase65d_phase62_contract["group_blocked_repeat_group_to_fold"],
        dtype=np.int64,
    )

phase65b_require(
    phase65d_group_to_fold.shape
    == (PHASE65D_CONFIG["repeat_count"], phase65c_expected_group_count),
    "phase65d_group_repeat_shape_mismatch",
)
phase65b_require(
    np.all(np.isin(phase65d_group_to_fold, [0, 1, 2])),
    "phase65d_group_repeat_value_invalid",
)
phase65d_repeat_fold = phase65d_group_to_fold[:, phase65c_groups]
phase65b_require(
    phase65d_repeat_fold.shape
    == (PHASE65D_CONFIG["repeat_count"], phase65c_expected_n),
    "phase65d_case_repeat_shape_mismatch",
)
for repeat in range(PHASE65D_CONFIG["repeat_count"]):
    phase65b_require(
        np.array_equal(np.unique(phase65d_repeat_fold[repeat]), np.arange(3)),
        "phase65d_repeat_missing_fold",
    )
    for group in range(phase65c_expected_group_count):
        phase65b_require(
            np.unique(
                phase65d_repeat_fold[repeat, phase65c_groups == group]
            ).size == 1,
            "phase65d_repeat_group_leakage",
        )


# Confirm that the exact Phase65C model/config lineage still exists. Only
# metadata and model-state structure are inspected; original Phase65C OOF
# probabilities are not read by Phase65D.
phase65d_phase65c_model_lineage_verified = phase65c_is_test
if not phase65c_is_test:
    expected_phase65c_config = {
    ...
    for fold in range(3):
        ...
        phase65b_require(
            model_path.is_file(),
            "phase65c_model_lineage_checkpoint_missing",
        )
        payload = torch.load(
        ...
        phase65b_require(
            ...
            and payload.get("run_sha256") == phase65c_run_sha256
            and int(payload.get("fold", -1)) == fold,
            "phase65c_model_lineage_run_mismatch",
        )
        phase65b_require(
            payload.get("phase65c_config") == expected_phase65c_config,
            "phase65c_model_lineage_config_mismatch",
        )
        phase65b_require(
            payload.get("architecture_config")
            == phase57c_architecture_config,
            "phase65c_model_lineage_architecture_mismatch",
        )
        phase65b_require(
            isinstance(payload.get("ema_model_state"), dict)
            and bool(payload["ema_model_state"]),
            "phase65c_model_lineage_state_missing",
        )
        del payload
    phase65d_phase65c_model_lineage_verified = True


phase65d_run_core = {
    "schema_version": PHASE65D_CONFIG["schema_version"],
    "phase65c_frozen_run_sha256": phase65c_run_sha256,
    "phase65c_contract_sha256": phase65d_phase65c_contract_sha256,
    "phase65c_unchanged_config": {
        key: value for key, value in PHASE65C_CONFIG.items()
        if key not in {"contract_file", "checkpoint_directory"}
    },
    "phase65d_config": {
        key: value for key, value in PHASE65D_CONFIG.items()
        if key not in {"contract_file", "checkpoint_directory"}
    },
    "group_to_fold": phase65d_group_to_fold.tolist(),
    "label_sha256": phase65c_array_sha256(phase65c_labels.astype(np.int8)),
    "group_sha256": phase65c_array_sha256(phase65c_groups.astype(np.int8)),
    "anchor_sha256": phase65c_array_sha256(phase65c_anchor.astype(np.float64)),
}
phase65d_run_sha256 = hashlib.sha256(json.dumps(
    phase65d_run_core, sort_keys=True, separators=(",", ":")
).encode("utf-8")).hexdigest()
phase65d_checkpoint_directory = (
    Path(PHASE65D_CONFIG["checkpoint_directory"])
    / phase65d_run_sha256[:16]
)
phase65d_checkpoint_directory.mkdir(parents=True, exist_ok=True)
phase65d_completed_fits_resumed = []
phase65d_epoch_resume = {}
phase65d_invalid_checkpoint_count = 0


def phase65d_fit_one(repeat, fold, train_indices, valid_indices):
    global phase65d_invalid_checkpoint_count
    ...
    oof_path = fit_directory / "oof.npz"
    model_path = fit_directory / "model.pt"
    training_path = fit_directory / "training.pt"

    if oof_path.is_file() and model_path.is_file():
        try:
            with np.load(oof_path, allow_pickle=False) as checkpoint:
                ...
                domain_accuracy = float(np.asarray(
            ...
            phase65b_require(
                checkpoint_run == phase65d_run_sha256
                and payload.get("run_sha256") == phase65d_run_sha256
                and int(payload.get("repeat", -1)) == repeat
                and int(payload.get("fold", -1)) == fold,
                "phase65d_completed_checkpoint_run_mismatch",
            )
            phase65b_require(
                np.array_equal(checkpoint_indices, valid_indices),
                "phase65d_completed_checkpoint_indices_mismatch",
            )
            phase65b_require(
                probability.shape == (valid_indices.size,)
                and np.all(np.isfinite(probability))
                and np.all((probability > 0.0) & (probability < 1.0)),
                "phase65d_completed_checkpoint_probability_invalid",
            )
            phase65b_require(
                loss_history.shape == (PHASE65C_CONFIG["epoch_count"],)
                and np.all(np.isfinite(loss_history)),
                "phase65d_completed_checkpoint_loss_invalid",
            )
            phase65d_completed_fits_resumed.append({
            ...
            row = {
                ...
                "final_domain_accuracy": float(domain_accuracy),
                "chance_domain_accuracy": float(1.0 / train_groups.size),
            }
            del payload
            return probability, row
        except Exception:
            phase65d_invalid_checkpoint_count += 1

    # Exact Phase65C seed policy: the seed depends only on the fold number.
    # Repeat number changes the training groups, not the declared randomness.
    prevalence = float(np.mean(phase65c_labels[train_indices]))
    model_seed = PHASE65C_CONFIG["seed"] + 1000 * fold
    ...
    def make_optimizer(current_model):
        return torch.optim.AdamW([
            {
                "params": current_model.backbone.encoder.parameters(),
                "lr": PHASE65C_CONFIG["encoder_learning_rate"],
            },
            {
                "params": current_model.backbone.head.parameters(),
                "lr": PHASE65C_CONFIG["disease_head_learning_rate"],
            },
            {
                "params": current_model.domain_heads.parameters(),
    ...
    train_dataset = Phase65CRealDataset(
        train_indices,
        two_views=True,
        seed=PHASE65C_CONFIG["seed"] + 20_000 * fold,
    )
    ...
    if training_path.is_file():
        try:
            ...
            phase65b_require(
                checkpoint.get("run_sha256") == phase65d_run_sha256
                and int(checkpoint.get("repeat", -1)) == repeat
                and int(checkpoint.get("fold", -1)) == fold,
                "phase65d_training_checkpoint_run_mismatch",
            )
            completed_epoch = int(checkpoint["completed_epoch"])
            phase65b_require(
                0 <= completed_epoch < PHASE65C_CONFIG["epoch_count"],
                "phase65d_training_checkpoint_epoch_invalid",
            )
            model.load_state_dict(checkpoint["model_state"], strict=True)
            ema_model.load_state_dict(checkpoint["ema_state"], strict=True)
            optimizer.load_state_dict(checkpoint["optimizer_state"])
            restored_histories = checkpoint["histories"]
            phase65b_require(
                set(restored_histories) == set(histories),
                "phase65d_training_checkpoint_history_keys_invalid",
            )
            histories = {
                key: [float(value) for value in restored_histories[key]]
                for key in histories
            }
            phase65b_require(
                all(len(value) == completed_epoch + 1
                    for value in histories.values()),
                "phase65d_training_checkpoint_history_length_invalid",
            )
            start_epoch = completed_epoch + 1
            global_step = start_epoch * steps_per_epoch
            phase65d_epoch_resume[f"{repeat}:{fold}"] = int(start_epoch)
        except Exception:
            phase65d_invalid_checkpoint_count += 1
            model = Phase65CDomainInvariantBilateral3D(
                seed=model_seed,
    ...
    for epoch in range(start_epoch, PHASE65C_CONFIG["epoch_count"]):
        ...
        loader = phase65c_loader(
            train_dataset,
            shuffle=True,
            seed=PHASE65C_CONFIG["seed"] + fold * 100_000 + epoch,
        )
        model.train()
        totals = {
            "total": 0.0, "classification": 0.0, "domain": 0.0,
            "consistency": 0.0,
        }
        domain_correct = 0
        case_count = 0
        ...
        for batch in loader:
            ...
            phase65b_require(
                ...
                "phase65d_training_group_not_in_fold_map",
            )
            ...
            lr_multiplier = phase65c_lr_multiplier(global_step, total_steps)
            for parameter_group, base_lr in zip(
                optimizer.param_groups, base_learning_rates
            ):
                parameter_group["lr"] = base_lr * lr_multiplier
            ...
            phase65b_require(
                ...
                "phase65d_nonfinite_training_loss",
            )
            total_loss.backward()
            ...
            totals["total"] += float(total_loss.detach().cpu()) * batch_n
            totals["classification"] += float(
                classification_loss.detach().cpu()
            ) * batch_n
            totals["domain"] += float(domain_loss.detach().cpu()) * batch_n
            totals["consistency"] += float(
                consistency_loss.detach().cpu()
            ) * batch_n
            domain_correct += int(torch.sum(
        ...
        for key in totals:
            histories[key].append(totals[key] / max(1, case_count))
        histories["domain_accuracy"].append(
            domain_correct / max(1, case_count)
        )
        phase65c_atomic_torch_save({
            "run_sha256": phase65d_run_sha256,
            "repeat": int(repeat),
            "fold": int(fold),
        ...
        print(
            f"Phase65D repeat {repeat + 1}/{PHASE65D_CONFIG['repeat_count']} "
            f"fold {fold + 1}/3 epoch {epoch + 1}/"
            f"{PHASE65C_CONFIG['epoch_count']}: "
            f"class={histories['classification'][-1]:.5f}, "
            f"domain_acc={histories['domain_accuracy'][-1]:.3f}",
            flush=True,
        )

    probability = phase65c_predict(
        ema_model,
        valid_indices,
        PHASE65C_CONFIG["seed"] + 900_000 + fold,
    )
    phase65c_atomic_torch_save({
        "run_sha256": phase65d_run_sha256,
        "phase65c_frozen_run_sha256": phase65c_run_sha256,
    ...
    phase65c_atomic_npz(
        oof_path,
        run_sha256=np.asarray(phase65d_run_sha256),
        valid_indices=valid_indices.astype(np.int64),
        probability=probability.astype(np.float64),
        loss_history=np.asarray(histories["total"], dtype=np.float64),
        final_domain_accuracy=np.asarray(
            histories["domain_accuracy"][-1], dtype=np.float64
        ),
    )
    row = {
        "repeat": int(repeat),
        ...
        "final_total_loss": float(histories["total"][-1]),
        "final_classification_loss": float(histories["classification"][-1]),
        "final_domain_loss": float(histories["domain"][-1]),
        "final_consistency_loss": float(histories["consistency"][-1]),
        "final_domain_accuracy": float(histories["domain_accuracy"][-1]),
        "chance_domain_accuracy": float(1.0 / train_groups.size),
    }

    del (
        model, ema_model, optimizer, train_dataset, loader,
...
phase65d_expert_by_repeat = np.full(
    (PHASE65D_CONFIG["repeat_count"], phase65c_expected_n),
    np.nan,
    dtype=np.float64,
)
...
for repeat in range(PHASE65D_CONFIG["repeat_count"]):
    for fold in range(PHASE65D_CONFIG["fold_count"]):
        valid_indices = np.flatnonzero(phase65d_repeat_fold[repeat] == fold)
        train_indices = np.flatnonzero(phase65d_repeat_fold[repeat] != fold)
        probability, fit_row = phase65d_fit_one(
            repeat, fold, train_indices, valid_indices
        )
        phase65d_expert_by_repeat[repeat, valid_indices] = probability
        phase65d_fit_rows.append(fit_row)

phase65b_require(
    np.all(np.isfinite(phase65d_expert_by_repeat))
    and np.all(
        (phase65d_expert_by_repeat > 0.0)
        & (phase65d_expert_by_repeat < 1.0)
    ),
    "phase65d_repeated_oof_incomplete",
)
phase65d_candidate_by_repeat = phase65c_sigmoid(
    PHASE65C_CONFIG["anchor_logit_weight"]
    * phase65c_logit(phase65c_anchor)[None, :]
    + PHASE65C_CONFIG["domain_invariant_expert_logit_weight"]
    * phase65c_logit(phase65d_expert_by_repeat)
)
phase65d_mean_expert = np.mean(phase65d_expert_by_repeat, axis=0)
phase65d_mean_candidate = np.mean(phase65d_candidate_by_repeat, axis=0)

phase65d_anchor_metrics = phase65c_metrics(
    phase65c_labels, phase65c_anchor
)
phase65d_mean_expert_metrics = phase65c_metrics(
    phase65c_labels, phase65d_mean_expert
)
phase65d_pooled = phase65c_compare(
    phase65c_labels, phase65c_anchor, phase65d_mean_candidate
)

phase65d_repeat_rows = []
phase65d_repeat_gains = []
phase65d_repeat_auc_gains = []
phase65d_fold_rows = []
phase65d_fold_regrets = []
for repeat in range(PHASE65D_CONFIG["repeat_count"]):
    comparison = phase65c_compare(
        phase65c_labels,
        phase65c_anchor,
        phase65d_candidate_by_repeat[repeat],
    )
    phase65d_repeat_rows.append({"repeat": int(repeat), **comparison})
    phase65d_repeat_gains.append(comparison["log_loss_gain"])
    phase65d_repeat_auc_gains.append(comparison["auroc_gain"])
    for fold in range(PHASE65D_CONFIG["fold_count"]):
        mask = phase65d_repeat_fold[repeat] == fold
        fold_comparison = phase65c_compare(
            phase65c_labels[mask],
            phase65c_anchor[mask],
            phase65d_candidate_by_repeat[repeat, mask],
        )
        phase65d_fold_rows.append({
            "repeat": int(repeat), "fold": int(fold), **fold_comparison
        })
        phase65d_fold_regrets.append(max(
            0.0, -fold_comparison["log_loss_gain"]
        ))

phase65d_repeat_gains = np.asarray(
    phase65d_repeat_gains, dtype=np.float64
)
phase65d_repeat_auc_gains = np.asarray(
    phase65d_repeat_auc_gains, dtype=np.float64
)
phase65d_max_repeat_regret = float(max(
    0.0, -np.min(phase65d_repeat_gains)
))
phase65d_max_fold_regret = float(max(phase65d_fold_regrets))

phase65d_major_mask = np.isin(
    phase65c_groups, PHASE65D_CONFIG["major_groups"]
)
phase65d_major_combined = phase65c_compare(
    phase65c_labels[phase65d_major_mask],
    phase65c_anchor[phase65d_major_mask],
    phase65d_mean_candidate[phase65d_major_mask],
)
phase65d_major_rows = []
phase65d_major_regrets = []
for group in PHASE65D_CONFIG["major_groups"]:
    mask = phase65c_groups == group
    comparison = phase65c_compare(
        phase65c_labels[mask],
        phase65c_anchor[mask],
        phase65d_mean_candidate[mask],
    )
    phase65d_major_rows.append({"group": int(group), **comparison})
    phase65d_major_regrets.append(max(0.0, -comparison["log_loss_gain"]))
phase65d_max_major_regret = float(max(phase65d_major_regrets))

phase65d_domain_rows = []
phase65d_domain_gains = []
for domain, definition in enumerate(phase65c_domain_definitions):
    ...
    phase65d_domain_rows.append(row)
    phase65d_domain_gains.append(comparison["log_loss_gain"])
phase65d_domain_gains = np.asarray(phase65d_domain_gains, dtype=np.float64)

phase65d_bootstrap_rng = np.random.default_rng(
    PHASE65D_CONFIG["domain_bootstrap_seed"]
)
phase65d_bootstrap = np.mean(
    phase65d_domain_gains[phase65d_bootstrap_rng.integers(
        0,
        phase65d_domain_gains.size,
        size=(
            PHASE65D_CONFIG["domain_bootstrap_replicates"],
            phase65d_domain_gains.size,
        ),
    )],
    axis=1,
)
phase65d_q025, phase65d_q50, phase65d_q975 = np.quantile(
    phase65d_bootstrap, [0.025, 0.50, 0.975]
).tolist()
phase65d_domain_rows_sorted = sorted(
    phase65d_domain_rows, key=lambda row: row["log_loss_gain"]
)

phase65d_gate = PHASE65D_CONFIG["advancement_gate"]
phase65d_advanced = bool(
    phase65d_pooled["log_loss_gain"]
    >= phase65d_gate["minimum_averaged_oof_log_loss_gain"]
    and phase65d_pooled["auroc_gain"]
    >= phase65d_gate["minimum_averaged_oof_auroc_gain"]
    and int(np.sum(phase65d_repeat_gains > 0.0))
    >= phase65d_gate["minimum_repeat_log_loss_wins"]
    and int(np.sum(phase65d_repeat_auc_gains > 0.0))
    >= phase65d_gate["minimum_repeat_auroc_wins"]
    and phase65d_max_repeat_regret
    <= phase65d_gate["maximum_repeat_log_loss_regret"]
    and phase65d_max_fold_regret
    <= phase65d_gate["maximum_individual_fold_log_loss_regret"]
    and phase65d_major_combined["log_loss_gain"]
    >= phase65d_gate["minimum_major_groups_combined_log_loss_gain"]
    and phase65d_max_major_regret
    <= phase65d_gate["maximum_individual_major_group_log_loss_regret"]
    and phase65d_q025
    >= phase65d_gate["minimum_domain_bootstrap_lower_95_log_loss_gain"]
    and phase65d_pooled["brier_gain"]
    >= -phase65d_gate["brier_regret_allowed"]
)
phase65d_status = (
    "phase65d_unchanged_candidate_repeated_group_confirmation_passed_ready_for_phase65e_deployment_refit"
    if phase65d_advanced
    else "phase65d_unchanged_candidate_repeated_group_confirmation_failed_retain_phase56"
)

phase65d_contract_core = {
    "schema_version": PHASE65D_CONFIG["schema_version"],
    "status": phase65d_status,
    "phase65c_contract_sha256": phase65d_phase65c_contract_sha256,
    "phase65c_frozen_run_sha256": phase65c_run_sha256,
    "phase65d_frozen_run_sha256": phase65d_run_sha256,
    "candidate": (
        "unchanged_sigmoid_0.85_anchor_logit_plus_0.15_"
        "phase65c_domain_invariant_expert_logit"
    ),
    "phase65c_hyperparameters_changed": False,
    "repeat_assignments_frozen_before_phase65c": True,
...
phase65d_contract_sha256 = hashlib.sha256(json.dumps(
    phase65d_contract_core, sort_keys=True, separators=(",", ":")
).encode("utf-8")).hexdigest()
phase65d_contract_path = Path(PHASE65D_CONFIG["contract_file"])
phase65d_contract_path.parent.mkdir(parents=True, exist_ok=True)
phase65d_contract_temporary = phase65d_contract_path.with_suffix(
    phase65d_contract_path.suffix + ".tmp"
)
phase65d_contract_temporary.write_text(json.dumps({
    **phase65d_contract_core,
    "contract_sha256": phase65d_contract_sha256,
...
os.replace(phase65d_contract_temporary, phase65d_contract_path)

PHASE65D_REPEATED_CONFIRMATION_STATE_PRIVATE = {
    "status": phase65d_status,
    "contract_sha256": phase65d_contract_sha256,
    "expert_oof_by_repeat": phase65d_expert_by_repeat.copy(),
    "candidate_oof_by_repeat": phase65d_candidate_by_repeat.copy(),
    "mean_candidate_oof_probability": phase65d_mean_candidate.copy(),
}

phase65d_report = {
    "phase": "phase65d_standalone_unchanged_phase65c_repeated_whole_group_confirmation",
    "status": phase65d_status,
    "restart_safety": {
        "requires_previous_notebook_cells": False,
        "requires_live_kernel_variables_in_production": False,
        "production_restore_used": bool(PHASE65B_PRODUCTION_RESTORE_USED),
        "frozen_run_sha256": phase65d_run_sha256,
        "completed_fit_count_resumed": int(
            len(phase65d_completed_fits_resumed)
        ),
        "completed_fits_resumed": phase65d_completed_fits_resumed,
        "epoch_resume": phase65d_epoch_resume,
        "invalid_or_stale_private_checkpoint_count": int(
            phase65d_invalid_checkpoint_count
        ),
    ...
    "source_integrity": {
        ...
        "phase62_contract_sha256": str(
            phase65c_validation["contract_sha256"]
        ),
        "phase65c_contract_sha256": phase65d_phase65c_contract_sha256,
        "phase65c_frozen_run_sha256": phase65c_run_sha256,
        "phase65c_failure_decision_preserved": True,
        "phase65c_model_lineage_verified": bool(
            phase65d_phase65c_model_lineage_verified
        ),
    ...
    "candidate_freeze": {
        ...
        "architecture_changed_from_phase65c": False,
        "losses_changed_from_phase65c": False,
        "epochs_changed_from_phase65c": False,
        "gradient_reversal_changed_from_phase65c": False,
        "augmentations_changed_from_phase65c": False,
        "blend_changed_from_phase65c": False,
        "fold_seed_policy_changed_from_phase65c": False,
        "calibration_or_router_added": False,
    },
    "validation": {
        "repeat_count": int(PHASE65D_CONFIG["repeat_count"]),
        "fold_count_per_repeat": int(PHASE65D_CONFIG["fold_count"]),
        "fit_count": int(
            PHASE65D_CONFIG["repeat_count"] * PHASE65D_CONFIG["fold_count"]
        ),
        "held_out_unit": "complete_acquisition_groups",
        "repeat_assignments_generated_without_labels": True,
        "repeat_assignments_frozen_before_phase65c": True,
        "phase65c_original_oof_predictions_used_for_fitting": False,
        "epoch_or_blend_selection_on_confirmation_labels": False,
        "fit_training": phase65d_fit_rows,
    },
    "anchor": phase65d_anchor_metrics,
    "mean_domain_invariant_expert_alone": phase65d_mean_expert_metrics,
    "averaged_eligible_candidate": phase65d_pooled,
    "repeats": phase65d_repeat_rows,
    "fold_extremes": {
        "worst_three": sorted(
            phase65d_fold_rows, key=lambda row: row["log_loss_gain"]
        )[:3],
        "best_three": sorted(
            phase65d_fold_rows, key=lambda row: row["log_loss_gain"],
            reverse=True,
    ...
    "major_groups": {
        "combined": phase65d_major_combined,
        "individual": phase65d_major_rows,
        "maximum_log_loss_regret": phase65d_max_major_regret,
    },
    "stress_domain_extremes": {
        "worst_three": phase65d_domain_rows_sorted[:3],
        "best_three": phase65d_domain_rows_sorted[-3:][::-1],
    },
    "domain_stability": {
        "domain_macro_log_loss_gain": float(np.mean(phase65d_domain_gains)),
        "domain_win_count": int(np.sum(phase65d_domain_gains > 0.0)),
        "domain_count": int(phase65d_domain_gains.size),
        "bootstrap_replicates": int(
            PHASE65D_CONFIG["domain_bootstrap_replicates"]
        ),
        "bootstrap_lower_95_log_loss_gain": float(phase65d_q025),
        "bootstrap_median_log_loss_gain": float(phase65d_q50),
        "bootstrap_upper_95_log_loss_gain": float(phase65d_q975),
    },
    "gate_summary": {
        "repeat_log_loss_wins": int(np.sum(phase65d_repeat_gains > 0.0)),
        "repeat_auroc_wins": int(np.sum(phase65d_repeat_auc_gains > 0.0)),
        "maximum_repeat_log_loss_regret": phase65d_max_repeat_regret,
        "maximum_individual_fold_log_loss_regret": phase65d_max_fold_regret,
        "maximum_major_group_log_loss_regret": phase65d_max_major_regret,
        "domain_bootstrap_lower_95_log_loss_gain": float(phase65d_q025),
        "thresholds": dict(phase65d_gate),
    },
    "interpretation_contract": {
        "phase65c_original_gate_remains_failed": True,
        "this_is_a_new_unchanged_candidate_robustness_adjudication": True,
        "failed_confirmation_requires_phase56_retention": True,
        "passing_confirmation_requires_phase65e_refit_and_offline_replay": True,
        "each_oof_model_excludes_complete_held_out_groups": True,
        "test_cases_processed_independently": True,
        "no_test_retraining_adaptation_or_batch_statistics": True,
        "leaderboard_used_for_fitting_or_selection": False,
    },
    ...
    "contract_sha256": phase65d_contract_sha256,
    "contract_file": str(phase65d_contract_path),
    "elapsed_seconds": float(time.perf_counter() - phase65d_started),
}

PHASE65D_REPEATED_CONFIRMATION_REPORT_PRIVATE = phase65d_report

print("BEGIN SANITIZED_PHASE65D_UNCHANGED_CANDIDATE_REPEATED_GROUP_CONFIRMATION")
print(json.dumps(phase65d_report, indent=2, sort_keys=False))
print("END SANITIZED_PHASE65D_UNCHANGED_CANDIDATE_REPEATED_GROUP_CONFIRMATION")
